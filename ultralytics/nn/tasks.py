...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if isinstance(x, tuple) and not isinstance(m, (CSWinStage, CSWinDownsample, Reshape)):
                x = x[0]
            ...
            if visualize:
                # Lấy tensor để visualize, kể cả khi x là tuple
                feature_tensor_for_vis = x[0] if isinstance(x, tuple) else x
                feature_visualization(feature_tensor_for_vis, m.type, m.i, save_dir=visualize)
            if embed and m.i in embed:
                # Lấy tensor để embed, kể cả khi x là tuple
                feature_tensor_for_embed = x[0] if isinstance(x, tuple) else x
                embeddings.append(nn.functional.adaptive_avg_pool2d(feature_tensor_for_embed, (1, 1)).squeeze(-1).squeeze(-1))
                if m.i == max(embed):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        ### SỬA ĐỔI 3: Thêm logic để xử lý các module CSWin ###
        ...
        if m in {
            ...
            BottleneckCSP, C1, C2, C2f, RepNCSPELAN4, ELAN1, ADown, AConv, SPPELAN, C2fAttn, C3, C3TR, C3Ghost,
        ...
        # Kết thúc các khối logic cho CSWin
            ...
            c1, cm, c2_ = ch[f], args[0], args[1]
            args = [c1, cm, c2_, *args[2:]]
            if m is HGBlock:
                args.insert(4, n)
                n = 1
            c2 = c2_
            ...
            args = [current_ch_in[0] if isinstance(current_ch_in, tuple) else current_ch_in]
            
        elif m is Concat:
            c2 = sum(x[0] if isinstance(x, tuple) else x for x in current_ch_in)

        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
            c_in = [x[0] if isinstance(x, tuple) else x for x in current_ch_in]
            args.append(c_in)
            if m is Segment:
            ...
            c2 = current_ch_in[-1][0] if isinstance(current_ch_in[-1], tuple) else current_ch_in[-1]

        else:
            c2 = current_ch_in[0] if isinstance(current_ch_in, tuple) else current_ch_in
